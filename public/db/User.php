    ...
    public function read(int $id = null): ?array
    {
        try {
            if ($id !== null) {
                ...
                $query = "SELECT User.ID, User.username, User.passwordHash, User.acc_typ, Mannschaft.Teamname, Station.name 
                  FROM User 
                  LEFT JOIN Mannschaft ON User.mannschaft_ID = Mannschaft.ID 
                  LEFT JOIN Station ON User.station_ID = Station.ID";
        ...
        try {
            ...
            if (!$confirmUpdate) {
                if ($foundName) {
                    return [
                        'status' => 'duplicate',
                        'message' => 'Ein Nutzer mit dem Namen existiert bereits. Möchtest du ihn aktualisieren?',
